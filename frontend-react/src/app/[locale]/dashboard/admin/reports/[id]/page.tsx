            ...
            case 1:
                return (
                    ...
                    >
                        ...
                        {/* Technical Specs Section (Integrated into Step 1) */}
                        <div className="pt-12 border-t border-black/5">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 px-4 md:px-11">
                                {[
                ...
                return (
                    ...
                    >
                        <div className="text-center space-y-4 max-w-2xl mx-auto">
                            ...
                            <p className="text-secondary/60 font-medium">يمكنك الآن طباعة التقرير، مشاركته مع العميل، أو تحديث حالته النهائية لإغلاق الطلب.</p>
                        </div>
            ...
            <div className="min-h-screen bg-[#FDFDFF] text-secondary selection:bg-primary/10 selection:text-primary relative p-4 md:p-8 overflow-x-hidden">
                ...
                <div className="relative max-w-7xl mx-auto space-y-8 md:space-y-16">
                    ...
                    <header className={cn(
                        ...
                        {/* <div className="flex items-center gap-3"> */}
                        {/* <span className="px-4 py-1.5 rounded-full bg-primary/10 text-primary text-[10px] font-black uppercase tracking-[0.2em] shadow-sm">Report Analysis</span> */}
                        {/* <span className="w-1.5 h-1.5 rounded-full bg-secondary/20" /> */}
                        {/* <span className="text-secondary/40 text-xs font-bold font-mono allign left-0">ID: {id}</span>
                        </div> */}
                        {/* <div className="grid grid-cols-2 md:flex gap-3 md:gap-4">
                            <Button variant="outline" className="rounded-2xl border-black/5 bg-white shadow-sm text-xs md:text-sm px-3 md:px-6" icon={<Printer size={16} />}>طباعة التقرير</Button>
                            <Button className="rounded-2xl shadow-xl shadow-primary/20 text-xs md:text-sm px-3 md:px-6" icon={<Share2 size={16} />}>إرسال للعميل</Button>
                        </div> */}
                    </header>
                    ...
                    <div className="flex flex-col lg:flex-row gap-8 lg:gap-20 items-start">
                        ...
                        {/* Minimal Numeric Stepper (Desktop Sidebar) */}
                        <aside className="hidden lg:block lg:w-72 sticky top-24 space-y-12">
                            ...
                            {/* Quick Info Sidebar */}
                            {/* <div className="p-8 rounded-[2.5rem] bg-secondary text-white space-y-6 shadow-2xl shadow-secondary/20 relative overflow-hidden group">
                                ...
                                <div className="relative z-10">
                                    <p className="text-[10px] font-black text-white/40 uppercase tracking-widest mb-1">حالة الطلب الحالي</p>
                                    <div className="flex items-center gap-2">
                                ...
                                <div className="relative z-10 pt-4 border-t border-white/10 space-y-4">
                                    <div>
                                        <p className="text-[10px] font-black text-white/40 uppercase tracking-widest">المبلغ المستحق</p>
                                        <p className="text-xl font-black">{parseFloat(report.amount || 0).toLocaleString()} <span className="text-xs">ج.م</span></p>
                                    </div>
                                    <Button size="sm" variant="outline" className="w-full bg-white/5 border-white/10 hover:bg-white hover:text-secondary border-0 rounded-2xl h-11 text-xs">
                                        تعديل الحالة
                                    </Button>
                                </div>
                            </div> */}
                        ...
                        <main className="flex-1 min-w-0">
                            ...
                            <div className="fixed bottom-6 md:bottom-12 left-1/2 -translate-x-1/2 flex items-center gap-2 md:gap-4 p-2 md:p-3 bg-white/90 backdrop-blur-3xl rounded-full border border-black/5 shadow-2xl z-50 ring-1 ring-black/[0.02] w-[95%] md:w-auto max-w-lg justify-between md:justify-center">
                                ...
                                <div className={cn(
                                    ...
                                    activeStep === 1 && "md:hidden" // Hide dots on step 1 to center the button better if needed
                                )}>
                                    {steps.map((s) => (
                                        <div
                                            key={s.id}
                                ...
                                <Button
                                    ...
                                    className={cn(
                                        ...
                                        activeStep === 1 && "flex-1 md:w-full max-w-none" // Full width on step 1
                                    )}
                                    onClick={() => {
                                        if (activeStep < 6) setActiveStep(prev => prev + 1);
                                        else router.push(`/${locale}/dashboard/admin/reports`);
                                    }}
                                >
                                ...
                                {({ zoomIn, zoomOut, resetTransform }) => (
                                    <>
                                        ...
                                        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[140] flex items-center gap-2 p-2 bg-white/10 backdrop-blur-2xl rounded-full border border-white/10 shadow-2xl">
                                            <button
                                                onClick={() => zoomIn()}
                                                className="w-10 h-10 flex items-center justify-center hover:bg-white/10 rounded-full text-white transition-all active:scale-90"
                                                title="Zoom In"
                                            >
                                                <Plus size={20} />
                                            </button>
                                        ...
                                        <TransformComponent
                                            wrapperClass="!w-screen !h-screen cursor-move"
                                            contentClass="w-full h-full flex items-center justify-center"
                                        >
                                            <img
                                                src={selectedImage}
                                                alt="Inspection Detail"
                                                className="max-w-[90%] max-h-[90%] object-contain rounded-2xl shadow-2xl pointer-events-none"
                                            />
                                        </TransformComponent>
                                    </>
                                )}
...
const getComponentNameArabic = (name: string) => {
    ...
    if (n.includes('front')) return 'الجهة الأمامية للطفاية';
    if (n.includes('back')) return 'الجهة الخلفية للطفاية';
    if (n.includes('left')) return 'الجهة اليسرى للطفاية';
    if (n.includes('right')) return 'الجهة اليمنى للطفاية';
    if (n.includes('bottom')) return 'الجهة السفلية للطفاية';
    if (n.includes('top')) return 'الجهة العلوية للطفاية';
...
const getComponentIcon = (name: string) => {
    ...
    if (lowerName.includes('cpu') || lowerName.includes('processor')) return <Cpu size={14} />;
    if (lowerName.includes('gpu') || lowerName.includes('graphics')) return <MonitorIcon size={14} />;
    if (lowerName.includes('battery') || lowerName.includes('power') || lowerName.includes('health')) return <Battery size={14} />;
    if (lowerName.includes('ssd') || lowerName.includes('hdd') || lowerName.includes('storage')) return <HardDrive size={14} />;
    if (lowerName.includes('keyboard')) return <Keyboard size={14} />;
    if (lowerName.includes('screen') || lowerName.includes('display') || lowerName.includes('lcd') || lowerName.includes('hdmi')) return <MonitorIcon size={14} />;
    if (lowerName.includes('ram') || lowerName.includes('memory')) return <Database size={14} />;
    if (lowerName.includes('wifi') || lowerName.includes('wi-fi')) return <Wifi size={14} />;
    if (lowerName.includes('bluetooth')) return <Bluetooth size={14} />;
    if (lowerName.includes('camera')) return <Camera size={14} />;
    if (lowerName.includes('mic')) return <Mic size={14} />;
    if (lowerName.includes('speaker')) return <Speaker size={14} />;
    if (lowerName.includes('touchpad') || lowerName.includes('mouse') || lowerName.includes('touchscreen')) return <Mouse size={14} />;
    if (lowerName.includes('usb') || lowerName.includes('port')) return <Cpu size={14} />;
    if (lowerName.includes('lan') || lowerName.includes('ethernet')) return <Wifi size={14} />;
    if (lowerName.includes('card reader')) return <CreditCard size={14} />;
    if (lowerName.includes('audio') || lowerName.includes('jack')) return <Speaker size={14} />;
    if (lowerName.includes('temp') || lowerName.includes('thermal')) return <Thermometer size={14} />;
    if (lowerName.includes('info') || lowerName.includes('system')) return <Info size={14} />;
    if (lowerName.includes('dxdiag')) return <Search size={14} />;
...
const getComponentTitle = (name: string) => {
    ...
    if (lowerName.includes('battery') && lowerName.includes('health')) return 'صحة البطارية (Battery Health)';
    if (lowerName === 'battery' || lowerName.includes('power')) return 'صحة وفحص البطارية';
    if (lowerName.includes('ssd') || lowerName.includes('hdd') || lowerName.includes('storage')) return 'فحص سرعة وحالة الهارد';
    if (lowerName.includes('keyboard')) return 'اختبار جميع أزرار الكيبورد';
    if (lowerName.includes('touchscreen')) return 'فحص شاشة اللمس (Touchscreen)';
    if (lowerName.includes('screen') || lowerName.includes('display') || lowerName.includes('lcd')) return 'فحص جودة الشاشة والبكسلات';
    if (lowerName.includes('hdmi')) return 'فحص منفذ HDMI';
    if (lowerName.includes('ram') || lowerName.includes('memory')) return 'اختبار استقرار الرامات';
    if (lowerName.includes('wifi') || lowerName.includes('wi-fi')) return 'اختبار الواي فاي (Wi-Fi)';
    if (lowerName.includes('lan') || lowerName.includes('ethernet')) return 'فحص منفذ الشبكة (LAN)';
    if (lowerName.includes('bluetooth')) return 'اختبار البلوتوث (Bluetooth)';
    if (lowerName.includes('camera')) return 'فحص الكاميرا';
    if (lowerName.includes('mic')) return 'اختبار المايكروفون';
    if (lowerName.includes('speaker')) return 'فحص السماعات والصوت';
    if (lowerName.includes('usb') && lowerName.includes('port')) return 'فحص منافذ USB';
    if (lowerName.includes('card reader')) return 'فحص قارئ البطاقات';
    if (lowerName.includes('audio jack')) return 'فحص منفذ السماعات';
    if (lowerName.includes('touchpad') || lowerName.includes('mouse')) return 'فحص التاتش باد';
    if (lowerName.includes('temp') || lowerName.includes('thermal')) return 'اختبار درجات الحرارة';
    if (lowerName.includes('info') || lowerName.includes('system')) return 'معلومات النظام والجهاز';
    if (lowerName.includes('dxdiag')) return 'تقرير DirectX (DxDiag)';
    if (lowerName.includes('motherboard') || lowerName.includes('logic board') || lowerName.includes('mainboard')) return 'فحص اللوحة الأم (Motherboard)';
    return name || 'فحص فني';
};
...
const getTestDescription = (name: string) => {
    ...
    if (comp.includes('cpu')) {
        return 'Stress Test للبروسيسور بيختبر قوة المعالج تحت ضغط تقيل، علشان يشوف لو هيقدر يشتغل بكفاءة في أقصى ظروف، وبيكشف لو في مشاكل زي السخونية أو الأداء الضعيف. يعني كأنك بتحط المعالج في "تمرين شاق" علشان تشوف هيستحمل ولا لأ..';
    }
    if (comp.includes('gpu')) {
        return 'برنامج FurMark بيعمل stress test لكارت الشاشة، يعني بيشغله بأقصى طاقته علشان يشوف هيسخن قد إيه ويقدر يستحمل الضغط ولا لأ. مفيد علشان تختبر التبريد وتشوف لو في مشاكل زي الحرارة العالية أو تهنيج الجهاز أثناء الألعاب او وقت الضغط.';
    }
    if (comp.includes('hdd') || comp.includes('ssd') || comp.includes('storage')) {
        return 'برنامج Hard Disk Sentinel بيكشف حالة الهارد، سواء HDD أو SSD، وبيقولك لو في مشاكل زي الباد سيكتور أو أداء ضعيف. كأنك بتعمل كشف شامل للهارد علشان تطمن إنه شغال تمام ومش هيفاجئك بعطل مفاجئ.';
    }
    if (comp.includes('battery')) {
        return 'الصورة دي لقطة من شاشة بتبين تفاصيل حالة بطارية اللابتوب، من خلال الـ BIOS. الهدف إنك تتأكد إن البطارية شغالة كويس وسليمة، يعني مش بتسخن أكتر من اللازم، ومش بتفقد شحن بسرعة، وبتدي الأداء اللي المفروض.';
    }
    if (comp.includes('keyboard')) {
        return 'اختبار زرار الكيبورد بيشوف إذا كانت كل الزراير شغالة صح ولا لأ. بتضغط على كل زر وبتشوف لو الجهاز بيستجيب، وده مفيد في ان نتأكد لو في زراير مش شغالة أو بتعلق.';
    }
    if (comp.includes('info') || comp.includes('system')) {
        return 'الشاشة اللي بتعرض معلومات الجهاز بتوريك حاجات زي ال (Serial Number) واللي عامل زي البصمة لكل لابتوب، نوع المعالج (CPU)، الرامات (Memory)، كارت الشاشة (GPU)، نسخة الـ BIOS، وكمان شوية معلومات عن النظام والتعريفات.';
    }
    if (comp.includes('dxdiag')) {
        return 'أداة dxdiag بتجمعلك معلومات عن الجهاز، زي كارت الشاشة، المعالج، الرامات، ونظام التشغيل، وكمان بتكشف لو في مشاكل في الـ DirectX. يعني باختصار، بتديك تقرير سريع عن حالة الجهاز.';
    }
    if (comp.includes('screen') || comp.includes('display') || comp.includes('lcd')) {
        return 'فحص دقيق للشاشة للتأكد من خلوها من أي بكسلات ميتة، خدوش، أو عيوب في الألوان والإضاءة، لضمان تجربة بصرية مثالية.';
    }
    if (comp.includes('ram') || comp.includes('memory')) {
        return 'اختبار شامل للرامات للتأكد من استقرارها وخلوها من الأخطاء، مما يضمن أداءً سلساً وموثوقاً للجهاز.';
    }
    if (comp.includes('wifi')) {
        return 'اختبار شامل لبطاقة الواي فاي للتأكد من قدرتها على الاتصال بالشبكات اللاسلكية بكفاءة وسرعة عالية.';
    }
    if (comp.includes('bluetooth')) {
        return 'فحص وظائف البلوتوث للتأكد من قدرة الجهاز على الاتصال بالأجهزة الأخرى لاسلكياً دون مشاكل.';
    }
    if (comp.includes('camera')) {
        return 'اختبار الكاميرا الأمامية والخلفية (إن وجدت) للتأكد من وضوح الصورة، عمل الميكروفون المدمج، وخلو العدسات من أي عيوب.';
    }
    if (comp.includes('mic')) {
        return 'فحص جودة الميكروفون للتأكد من التقاط الصوت بوضوح ودون تشويش، وهو أمر حيوي للمكالمات والتسجيلات.';
    }
    if (comp.includes('speaker')) {
        return 'اختبار السماعات الداخلية والخارجية (إن وجدت) للتأكد من نقاء الصوت، وسلامة جميع مخارج الصوت.';
    }
    if (comp.includes('touchpad') || comp.includes('mouse')) {
        return 'فحص دقيق للوحة اللمس (Touchpad) أو الماوس للتأكد من استجابتها السريعة والدقيقة للحركات والإيماءات.';
    }
    if (comp.includes('temp') || comp.includes('thermal')) {
        return 'مراقبة درجات حرارة المكونات الداخلية للجهاز تحت الضغط للتأكد من كفاءة نظام التبريد وتجنب السخونة الزائدة.';
    }
    if (comp.includes('motherboard') || comp.includes('logic board') || comp.includes('mainboard')) {
        return 'فحص شامل للوحة الأم للتأكد من سلامة جميع المكونات المتصلة بها، وخلوها من أي تلف أو عيوب قد تؤثر على أداء الجهاز.';
    }
                ...
                {selectedMedia ? (
                    selectedMedia.type === 'video' || selectedMedia.type === 'youtube' ? (
                        ...
                        <img
                            src={selectedMedia.url}
                            alt="Selected"
                            className="w-full h-full object-contain bg-black/90 cursor-zoom-in"
                            onClick={() => onImageClick(selectedMedia.url)}
                        />
                    )
                ) : (
            ...
            <div className="flex gap-2 md:gap-4 overflow-x-auto pb-4 px-1 snap-x no-scrollbar">
                {video && (
                    <button
                        ...
                        className={cn(
                            "flex-shrink-0 w-16 h-16 md:w-32 md:h-32 rounded-lg md:rounded-2xl overflow-hidden border-2 transition-all relative group snap-start",
                            selectedMedia === video ? "border-primary ring-2 md:ring-4 ring-primary/20 scale-105" : "border-transparent opacity-60 hover:opacity-100"
                        )}
                    >
                        <div className="absolute inset-0 bg-black/50 flex items-center justify-center group-hover:bg-black/40 transition-colors">
                            <Video className="text-white drop-shadow-lg" size={20} />
                        </div>
                ...
                {images.map((img: any, idx: number) => (
                    <button
                        ...
                        className={cn(
                            "flex-shrink-0 w-16 h-16 md:w-32 md:h-32 rounded-lg md:rounded-2xl overflow-hidden border-2 transition-all snap-start",
                            selectedMedia === img ? "border-primary ring-2 md:ring-4 ring-primary/20 scale-105" : "border-transparent opacity-60 hover:opacity-100"
                        )}
                    >
...
function InternalInspectionSection({ report, onImageClick }: { report: any, onImageClick: (url: string) => void }) {
    ...
    let media = [];
    try {
        hardwareTests = typeof report.hardware_status === 'string' ? JSON.parse(report.hardware_status) : (report.hardware_status || []);
        media = typeof report.external_images === 'string' ? JSON.parse(report.external_images) : (report.external_images || []);
    } catch (e) { }

    const testScreenshots = media.filter((m: any) => m.type === 'test_screenshot');

    const findStatus = (keywords: string[]) => {
        const item = hardwareTests.find((t: any) => keywords.some(k => t.componentName?.toLowerCase().includes(k)));
        return item ? { status: item.status, notes: item.notes, exists: true } : { status: 'Not Tested', notes: '-', exists: false };
    };

    const battery = findStatus(['battery', 'power']);
    const motherboard = findStatus(['motherboard', 'logic board', 'mainboard']);
    ...
    return (
        <div className="space-y-6">
            ...
            {/* <div className="flex flex-wrap gap-2">
                <div className="px-4 py-2 bg-green-50 border border-green-100 rounded-2xl flex items-center gap-2.5 shadow-sm">
                    <Battery size={14} className="text-green-600" />
                    <span className="text-xs font-bold text-secondary/40">البطارية:</span>
                    <span className="text-xs font-black text-green-700">{battery.status || 'ممتازة'}</span>
                </div>
                <div className="px-4 py-2 bg-blue-50 border border-blue-100 rounded-2xl flex items-center gap-2.5 shadow-sm">
                    <Cpu size={14} className="text-blue-600" />
                    <span className="text-xs font-bold text-secondary/40">لوحة الأم:</span>
                    <span className="text-xs font-black text-blue-700">{motherboard.status || 'سليمة'}</span>
                </div>
            </div> */}
            ...
            {testScreenshots.length > 0 ? (
                        ...
                        return (
                            <div
                                ...
                                className={cn(
                                    ...
                                    isOpen ? "border-primary/20 shadow-lg" : "border-black/5 hover:border-black/10 shadow-sm"
                                )}
                            >
                                {/* Accordion Header */}
                                <button
                                    onClick={() => setOpenIndex(isOpen ? null : idx)}
                                    className="w-full px-4 md:px-6 py-4 md:py-5 flex items-center justify-between group text-right"
                                >
                                    <div className="flex items-center gap-4">
                                        <div className={cn(
                                            "w-10 h-10 rounded-xl flex items-center justify-center transition-colors",
                                            isOpen ? "bg-primary text-white" : "bg-black/[0.03] text-secondary/40 group-hover:bg-primary/10 group-hover:text-primary"
                                        )}>
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[10px] text-secondary/40 font-bold uppercase tracking-wider">
                                                Advanced Technical Test | فحص فني متقدم
                                            </span>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        <Badge variant="outline" className="bg-green-50 text-green-700 border-green-100 text-[10px] font-black h-6 px-3 rounded-full">
                                            تم بنجاح (PASSED)
                                        </Badge>
                                        <div className={cn(
                                            "transition-transform duration-300",
                                            isOpen ? "rotate-90 text-primary" : "text-secondary/20"
                                        )}>
                                            <ChevronLeft size={20} />
                                        </div>
                                    </div>
                                    ...
                                    {isOpen && (
                                        ...
                                        >
                                            <div className="px-6 pb-8 pt-4 border-t border-black/[0.03]">
                                                <div
                                                    className="w-full aspect-video md:aspect-[21/9] bg-black/[0.02] rounded-2xl md:rounded-3xl overflow-hidden border border-black/5 cursor-zoom-in group/img relative"
                                                ...
                                                <div className="space-y-4 md:space-y-6">
                                                    <div className="p-4 md:p-6 rounded-2xl md:rounded-3xl bg-black/[0.02] border border-black/[0.03]">
                                                        ...
                                                        <p className="text-secondary/70 text-sm md:text-base leading-relaxed font-bold">
                                                            {item.comment || getTestDescription(componentName)}
                                                        </p>
                                                    </div>
                                                    <div className="flex items-center gap-3 md:gap-4 p-3 md:p-4 rounded-2xl md:rounded-3xl border border-green-100/50 bg-green-50/30">
                                                        <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-green-500 flex items-center justify-center text-white shadow-lg shadow-green-500/20 shrink-0">
                                                            <Check size={16} />
                                                        </div>
                                                        <span className="text-xs md:text-sm font-black text-green-700 italic">تم التحقق بنجاح - المكون يعمل بأفضل أداء</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </motion.div>
                ...
                <div className="py-12 text-center border-2 border-dashed border-black/5 rounded-[2.5rem] opacity-40">
                    <ImageIcon className="mx-auto mb-4 text-secondary/20" size={48} />
                    <p className="text-sm font-black text-secondary/40">لا توجد صور أو فحوصات تقنية مسجلة</p>
                </div>
            )}
        </div >
    );
