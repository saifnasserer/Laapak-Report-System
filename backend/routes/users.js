...
router.put('/admins/:id', adminAuth, async (req, res) => {
    ...
    try {
        ...
        // Only allow staff/admins to update other admins
        if (req.user.role !== 'admin' && req.user.role !== 'superadmin' && req.user.id !== admin.id) {
        ...
        if (req.body.username && req.user.role === 'superadmin') {
