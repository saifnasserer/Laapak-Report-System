...
router.post('/:id/share/whatsapp', auth, async (req, res) => {
    try {
        ...
        const phoneNumber = invoice.client?.phone;
        if (!phoneNumber) {
            return res.status(400).json({ message: 'رقم هاتف العميل غير موجود', error: 'Client phone number not found' });
        }
        ...
        // Dynamic print URL using current host
        const protocol = req.protocol;
        const host = req.get('host');
        const printUrl = `${protocol}://${host}/api/invoices/${id}/print?token=${token}`;

        // Custom Arabic message
        const message = `الفاتورة الخاصة بحضرتك من شركة لابك رقم ${id}.\n\nيمكنكم الاطلاع عليها من الرابط التالي:\n${printUrl}`;

        console.log(`Sharing invoice ${id} to ${phoneNumber} via WhatsApp...`);

        await Notifier.sendText(phoneNumber, message);

        res.json({ success: true, message: 'تم إرسال الفاتورة بنجاح عبر واتساب' });
    } catch (error) {
